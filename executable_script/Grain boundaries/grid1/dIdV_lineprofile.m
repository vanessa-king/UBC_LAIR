...
masks = data.grid1.directional_masksGB;
pixel_size_nm = 5 / 30; % size of topo / number of points
distance = pixel_size_nm * (0:(size(profile,2)-1));

for i = 1:size(masks, 3)
    mask = masks(:,:,i); % assuming masks is a cell array of logical masks
    mask_expanded = repmat(mask, [1 1 size(data.grid1.dIdV_smoothed, 3)]);
    masked_data = data.grid1.dIdV_smoothed.*mask_expanded; % apply the mask
...
% HOMO LUMO onset
threshold = 0.1;
[LUMO_onset, HOMO_onset] = find_gap(profile_smoothed, data.grid1.V_reduced, threshold, 5);

% (Optional) Plot the crossings
figure; hold on
imagesc(distance, data.grid1.V_reduced, profile_smoothed); 
set(gca, 'YDir', 'normal');
colormap('parula')
colorbar;
...
yline(1.15, 'LineStyle','--', 'LineWidth',0.1)
yline(-2.25, 'LineStyle','--', 'LineWidth',0.1)
hold off
title('Grid 1, GB')