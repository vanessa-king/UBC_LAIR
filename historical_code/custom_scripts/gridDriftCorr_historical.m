function [grdcorr] = gridDriftCorr(grd, before, after, theta)
...
%   so choose.
%   Input: 
%   - grd: original data structure from gridLoadData
...
temp = imwarp(squeeze(grd.iv(1,:,:)),tform);
iv = zeros(length(grd.V),size(temp,1), size(temp,2));
for i = 1:length(grd.V)
    iv(i,:,:) = imwarp(squeeze(grd.iv(i,:,:)),tform);
end

% redefine x and y axis, distances should be preserved
x_img = linspace(0,mean(diff(grd.x_img))*size(z_img,1),size(z_img,1));
y_img = linspace(0,mean(diff(grd.y_img))*size(z_img,2),size(z_img,2));
x = linspace(0,mean(diff(grd.x))*size(iv,2),size(iv,2));
y = linspace(0,mean(diff(grd.y))*size(iv,3),size(iv,3));
