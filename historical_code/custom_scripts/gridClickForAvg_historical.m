...
function [] = gridClickForAvg(didv,Vred,imV,R,n,offset,xysmooth, vsmooth, grd)
%----Inputs----
%IVdata: 3D Matrix with data
%V: Vector with bias voltages
%imV: Voltage at which to display image
%R: radius of points averaged, and drawn on the plot
...
%grd: grid data for didv, use this to plot the topographic information
    %rather than imV 
    
% all the conditional functions
...
didv_flip = flip(permute(didv,[1 3 2]),2); % flip didv so that image (x,y) = (0,0) at the bottom left
                                           % and the first axis is the x,
didv_xy = permute(didv, [2 3 1]);
                                           
% use cases for different function arguments
switch nargin
    case 9 % grd data is provided, use the topology instead
    ...
    case 8 % grd data not provided, use the didv slice at imV
...
for k = 1:n
    ...
    % generate mask for averaging
    mask = zeros(size(didv,2),size(didv,3));
    mask((-R:R)+pos(1),(-R:R)+(size(mask,2)-pos(2))) = logical(fspecial('disk',R));
    didv_masked = bsxfun(@times, didv_xy, mask);
    didv_avg = squeeze(sum(didv_masked,[1 2])/sum(mask,'all'));
    
    plot(Vred, didv_avg+(k-1)*offset,colours(mod(k-1,7)+1))
end
