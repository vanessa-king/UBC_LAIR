function [plot_names,Biases,comment] = gridPlotSlices_historical(I, V, Biases, plotname)
...
for k = 1:N
    ...
    % Create a new plot name for each figure
    plot_names{k} = [plotname{1}, 'Biases', num2str(Biases(k))];
    clims = [0, 3E-11];
    % Flip y-axis to correct mirroring
    slice = squeeze(permute(I(:, :, temp_ind), [2, 1, 3])); % (y, x)
    slice = flipud(slice); % Correct the y-axis orientation
    imagesc(slice, clims);
    colorbar;
    colormap(cm_magma);
    axis image;
