function [header, par, I, dIdV, LockindIdV, voltage, midV, QPI, LockinQPI] = load3dsall(fn, sigma)
...
voltage = linspace(par(1),par(2),header.points);
voltage = voltage';
...
for i = 1:x_num
    for j =1:y_num
         LockindIdV(i,j,:) = gaussfilter1d(data{i,j}(:,3),sigma); % Make sure to check which channel is Lockin signal sitting 
    end
end
