function [mask, comment, startPoint, endPoint, polcoord] = gridMaskLine(topo, plotmask, pointA, pointB, polcoord)
...
%   plotmask    indication if you want to plot the mask you created 
...
arguments
    topo
    plotmask    {mustBeNumericOrLogical}=true
...
if isempty(pointA) && isempty(pointB)  && isempty(polcoord)
    ...
    if method == "Y" || method == "y"
        ...
        % compute the polcoord from 2 points 
...
% make the mask (either way below works.)
%[mask,~] =createLineSegmentMask_modified(size(topo), BoundaryPoint1, BoundaryPoint2);
[mask,~] =createLineSegmentMask_Markus(size(topo), BoundaryPoint1, BoundaryPoint2);
