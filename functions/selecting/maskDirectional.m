function [masks, masks_combined, comment] = maskDirectional(data, n, connected, startPoint, endPoint, bin_size, bin_sep)
...
% Dec 2024 - Dong Chen; May 2025 - Rysa

arguments
    data        
    n           {mustBePositive} = []   % optional, only for 3D data
    connected   = []                    % optional, only for 3D data
    startPoint      = []                % optional, only for 3D data
    endPoint =[]
    bin_size=[]
    bin_sep=[]
end

% If input data is 3D set variable 'data' to slice n
if ndims(data) == 3
    data = data(:,:,n);
end
...
[masks, dir_comment] = maskSingleDirectional(data, connected, ...
    startPoint, endPoint);
